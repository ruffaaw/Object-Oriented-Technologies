...
public class FindNearestUnitStrategy implements IStrategy {
    ...
    private double countDistance(double[] alarmLocation, Unit unit) {
        return Math.sqrt((alarmLocation[0] - unit.getCords()[0]) * (alarmLocation[0] - unit.getCords()[0]) + (alarmLocation[1] - unit.getCords()[1]) * (alarmLocation[1] - unit.getCords()[1]));
    }
